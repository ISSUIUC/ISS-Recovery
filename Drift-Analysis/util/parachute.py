        ...
        while alt > end_altitude:
            ...
            if(abs(alt.m() - config.MAIN_ALTITUDE.m()) < config.DYNAMIC_TIMESTEP_RANGE.m()):
                ...
                if print_debounce > print_debounce_max:
                    print(f"simulating... {100*(1-alt.m()/config.APOGEE_ALTITUDE_SUSTAINER.m()):.0f}%  [Fine timestep]", end="    \r")
                    print_debounce = 0
            else:
                timestep = normal_timestep
                if print_debounce > print_debounce_max:
                    print(f"simulating... {100*(1-alt.m()/config.APOGEE_ALTITUDE_SUSTAINER.m()):.0f}%  [Normal timestep]", end="    \r")
                    print_debounce = 0
